...
class FileLock(object):
    ...
    def wait(self) -> bool:
        """ Wait release lock file
        true - lock found
        false - lock not exists
        exceptions timeout wait
        """
        exists = False
    ...
    def acquire(self):
        ...
        while True:
            try:
                ...
                mylog.debug(f"lock file aquire {self.lockfile}")
                break;
            except OSError as e:
...
#        self.is_locked = True
    ...
    def release(self):
        ...
        if self.is_locked:
            ...
            self.is_locked = False
            mylog.debug(f"lock file release {self.lockfile}")
