...
class DDS_Field(dict):
    ...
    REPLACES = {
        'ID object': 'Id',
        'Object is Marked for': 'Помечен',
        'Row ID': 'НомерСтроки',
        'LineNo': 'НомерСтроки',
        'Version stamp': 'Время',
        'object code': 'Код',
        'object description': 'Наименование',
        'Parent in other tabl': 'Parentext',
        'ID parent obj': 'Родитель',
    ...
    def desc(self):
        dsc = self.fieldstr[1]
        if dsc in DDS_Field.REPLACES:
            return DDS_Field.REPLACES[dsc]
        return self.fieldstr[1].replace('(P)','')
    ...
    def as_div(self):
        return '<div class="%s">%s, %s</div>' % (div_class, self.name, self.sql)
...
class DDS_Table(dict):
    ...
    def __init__(self, dds_obj):
        ...
        self.tablehead = dds_obj._curline_().split(':')[1].strip()#.replace(' (Дв.)', 'Движения')
    ...
    def as_div(self):
        r = '<div class="%s">%s ' % (div_class, self.name)
        r += "".join([fobj.as_div() for fobj in self.values()]) + '</div>'
        return r
...
class DDS(dict):
    ...
    def prepareSQL(self, sqltext, debug=False):
        ...
        errors = False
        sqlres = u"%s" % sqltext
        for m in re.finditer(r'\{([a-zа-я0-9\.^ ]+)\}|\$([a-zа-я0-9\.^ ]+)', sqltext, re.UNICODE|re.IGNORECASE):
